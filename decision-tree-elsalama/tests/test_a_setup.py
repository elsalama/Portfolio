...
def test_setup_netid():
    ...
    assert netid != "NETID_GOES_HERE", "Please add your NetID"
    assert netid.lower() == netid, "Please use lower-case letters for your Net ID"
    assert re.search(r"^[a-z]{3}[0-9]{3,4}$", netid) is not None, "Your NetID should look like xyz0123"
...
def test_setup_password():
    ...
    In the example above (but not in the actual assignment), `2da3e727` is your
    password. You need to add that to your `password` file, replacing the
    `autograder_password_goes_here` with a single line containing this
    password.  Note that when you try to commit and push this change, you may
    get a scary-looking git error such as:

    ```
       ! [rejected]        main -> main (fetch first)
      error: failed to push some refs to 'git@github.com:cs349/hw1-decision-tree-username.git'
